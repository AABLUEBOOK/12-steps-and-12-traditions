      ...
      <div className="bg-slate-700/50 hover:bg-slate-700/80 border border-slate-600/50 rounded-lg px-5 py-4 transition-all duration-200 flex items-center justify-between">
        <div className="flex items-center gap-3">
          {showNumber && displayNumber && (
            <span className="text-teal-400 font-mono text-sm min-w-[24px]">{displayNumber}</span>
          )}
          <span className="text-slate-100 font-medium tracking-wide">{title}</span>
        </div>
        <div className="flex items-center gap-3">
          {pages && <span className="text-slate-400 text-sm">{pages}</span>}
